...
class AopClient {
	...
	protected function curl($url, $postFields = null) {
		...
		if (curl_errno($ch)) {
			...
			die(1);//throw new Exception(curl_error($ch), 0);
		} else {
			$httpStatusCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
			if (200 !== $httpStatusCode) {
				die(1);//throw new Exception($reponse, $httpStatusCode);
			}
		}
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (strcasecmp($this->fileCharset, $this->postCharset)) {
			...
			die(1);//throw new Exception("文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!");
		}
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			if ($this->checkEmpty($apiParams['biz_content'])) {
				...
				die(1);//throw new Exception(" api request Fail! The reason : encrypt request is not supperted!");
			}

			if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

				die(1);//throw new Exception(" encryptType and encryptKey must not null! ");
			}

			if ("AES" != $this->encryptType) {

				die(1);//throw new Exception("加密类型只支持AES");
			}
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			...
			if ($signData == null || $this->checkEmpty($signData->sign) || $this->checkEmpty($signData->signSourceData)) {
				...
				die(1);//throw new Exception(" check sign Fail! The reason : signData is Empty");
			}
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
						...
						if (!$checkResult) {
							die(1);//throw new Exception("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
						}
